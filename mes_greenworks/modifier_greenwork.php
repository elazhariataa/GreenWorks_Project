...
if(isset($_GET["id"])){
    extract($_GET);
    try{
        $requestmod=$db->prepare("SELECT* FROM greenwork WHERE idgreenwork =? ");
        $requestmod->execute([$id]);
        $info= $requestmod->fetch();
    }
...
<body>
    <div class="continer">
        ...
        <div class="main">
            <div class="title"><h3>Modifier un GreenWork</h3></div>
            <?php
            ...
            <form name="form4" method="POST" enctype="multipart/form-data">
                <div>
                    ...
                    <input type="text" name="titre" id="idtitre" value="<?php echo $info[1] ?>">
                </div>
                <div>
                    <label for="ing">Ingrédients</label>
                    <textarea name="ing" id="iding"
                        rows="5"
                        cols="100"><?php echo $info[2] ?>
                    </textarea>
                </div>
                <div>
                    ...
                    <textarea name="etapes" id="idetapes"
                        rows="5"
                        cols="100"><?php echo $info[3] ?>
                    </textarea>
                </div>
                <div>
                    <label for="dateg">Date de creation de GreenWork: </label>
                    <input type="date" name ="dateg" id="iddateg" value="<?php echo $info[4] ?>">
                </div>
                <div>
                    <label for="photo">Photo de GreenWork: </label>
                    <input type="file" name ="photo" id="idphoto" value="<?php echo $info[5] ?>">
                </div>
                <div>
                    <input type="submit" name="modifier" value="Modifier" id="idmodifier">
                </div>
            </form>
        </div>
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if(!isset($titre) || empty($titre)){
        header("Location:modifier_greenwork.php?message=error_titre");
    }else{
        if(!isset($ing) || empty($ing)){
            header("Location:modifier_greenwork.php?message=error_ing");
        }else{
            if(!isset($etapes) || empty($etapes)){
                header("Location:modifier_greenwork.php?message=error_etapes");
            }else{
                if(!isset($dateg) || empty($dateg)){
                    header("Location:modifier_greenwork.php?message=error_dateg");
                }else{
                    ...
                    try{
                        $requesteMG = $db->prepare("UPDATE greenwork SET Titregreenwork=?,Inggreenwork=?,Etapesgreenwork=?,Dategreenwork=?,imagegreenwork =? WHERE 	idgreenwork = ?");
                        $requesteMG->execute([$titre,$ing,$etapes,$dateg,".\\uploaded_pictures\\".$_FILES["photo"]["name"],$id]);
